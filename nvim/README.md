...
- `lua/muxwf/init.lua`
  Main orchestration layer. Registers commands, default mappings, scratch-buffer list views, and high-level actions like `open`, `restore`, and `workspace_open`.
...
- `lua/muxwf/picker.lua`
  Picker/UI layer. Builds work and workspace entries, Telescope previews, and fallback `vim.ui.select()` flows.
